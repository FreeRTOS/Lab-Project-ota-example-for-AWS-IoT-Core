...
static const char * jobStatusString[5U] = {"Queued", "InProgress", "Failed", "Succeeded", "Rejected"};

static size_t prvGetStartNextPendingJobExecutionTopic( const char * thingname, size_t thingnameLength, char * buffer, size_t bufferSize );
static size_t prvGetStartNextPendingJobExecutionMsg( const char * clientToken, size_t clientTokenLength, char * buffer, size_t bufferSize );
static size_t prvGetUpdateJobExecutionTopic( char * thingname, size_t thingnameLength, char * jobId, size_t jobIdLength, char * buffer, size_t bufferSize );
static size_t prvGetUpdateJobExecutionMsg( JobStatus_t status, char * expectedVersion, size_t expectedVersionLength, char * buffer, size_t bufferSize );
...
bool startNextPendingJob(char * thingname, size_t thingnameLength, char * clientToken, size_t clientTokenLength)
{
    ...
    size_t topicLength = prvGetStartNextPendingJobExecutionTopic(thingname, thingnameLength, topicBuffer, TOPIC_BUFFER_SIZE);

    size_t messageLength = prvGetStartNextPendingJobExecutionMsg(clientToken, clientTokenLength, messageBuffer, MAXIMUM_MESSAGE_SIZE);

    if ( topicLength > 0 && messageLength > 0 )
    {
      published = mqttPublish(topicBuffer, topicLength, (uint8_t *) messageBuffer, messageLength);  
    }
...
bool updateJobStatus(char * thingname, size_t thingnameLength, char * jobId, size_t jobIdLength, JobStatus_t status, char * expectedVersion, size_t expectedVersionLength)
{
    ...
    size_t topicLength = prvGetUpdateJobExecutionTopic(thingname, thingnameLength, jobId, jobIdLength, topicBuffer, TOPIC_BUFFER_SIZE);

    size_t messageLength = prvGetUpdateJobExecutionMsg(status, expectedVersion, expectedVersionLength, messageBuffer, MAXIMUM_MESSAGE_SIZE);
...
static size_t prvGetStartNextPendingJobExecutionTopic( const char * thingname, size_t thingnameLength, char * buffer, size_t bufferSize )
{
    ...
    strncpy(buffer+topicLength, "/jobs/start-next", bufferSize - topicLength );
...
static size_t prvGetUpdateJobExecutionMsg( JobStatus_t status, char * expectedVersion, size_t expectedVersionLength, char * buffer, size_t bufferSize )
{
    ...
    strncpy(buffer+messageLength, jobStatusString[status], bufferSize-messageLength);
    messageLength += strlen(jobStatusString[status]);
    strncpy(buffer+messageLength, "\",\"expectedVersion\":\"", bufferSize-messageLength);
    strncpy(buffer+messageLength, expectedVersion, bufferSize-messageLength);
